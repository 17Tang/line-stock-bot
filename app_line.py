...
class LineWebhookHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # 1. 安全驗證：驗證這封訊息是不是真的從 LINE 官方發過來的
        ...
        # 2. 回應 LINE 伺服器 200 OK 告知已收到封包
        self.send_response(200)
        self.end_headers()

        # 3. 開啟非同步平行任務去處理股號計算，避免堵塞 LINE 伺服器
        try:
...
def process_and_reply_line(reply_token, user_text):
    if user_text.lower() == "hello" or user_text == "開始":
        send_line_reply(reply_token, "👋 歡迎使用關鍵價看盤助手！\n請直接輸入股票代號（例如：2330）即可查詢技術指標。")
        return

    try:
        p = calculate_stock_prices(user_text)
        if p is None:
            send_line_reply(reply_token, f"❌ 找不到股票代號 '{user_text}' 的資料，請確認是否輸入正確。")
            return
