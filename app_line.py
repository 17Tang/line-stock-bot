    ...
    for d in dates_to_fetch:
        try:
            if not is_tpex:
                ...
                # 櫃買中心：遷移至全新 inxh_result.php 路徑
                roc_ym = f"{d.year - 1911}/{d.strftime('%m')}"
                url = f"https://www.tpex.org.tw/zh-tw/indices/stock-index/industrial/inxh_result.php?l=zh-tw&d={roc_ym}"
                res = requests.get(url, headers=yf_session.headers, timeout=5).json()
                if "aaData" in res:
...
def fetch_live_index_patch(is_tpex=False):
    """
    當歷史資料庫產生更新時間差時，使用證交所 MiS 即時系統強制補齊今日最新的大盤K線數據。
    """
    try:
...
def calculate_stock_prices(stock_id):
    ...
    # 🚀 路線一：直連全新台灣官方歷史網址
    if is_tw_index:
        print("🟢 啟用方案 B：使用修正版台灣官方 API 獲取大盤資料...")
        df_daily = get_taiwan_official_index(is_tpex)
        
    # 🚀 路線二：若路線一回傳空值，退回備用 yfinance 歷史線
    if not is_tw_index or df_daily.empty:
        if is_tw_index:
            print("⚠️ 新版官方 API 異常，自動退回備用 Yahoo Finance 歷史資料")
        try:
    ...
    # 🚀 路線三（超強終極補丁）：若歷史數據落後今天，用官方 MiS 即時補齊今日K線
    if is_tw_index:
